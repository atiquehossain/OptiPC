...
class RecoveryPage(BasePage):
    ...
    def build(self) -> None:
        ...
        ctk.CTkLabel(
            ...
            text=(
                ...
                "2. Stop using the source drive as much as possible\n"
                "3. SSD recovery may fail if data was already overwritten"
            ),
            justify="left",
            wraplength=460,
            text_color="orange",
        ...
        ctk.CTkLabel(
            help_card,
            text=(
                "regular = best first try for recently deleted files on healthy NTFS drives.\n\n"
                "extensive = broader search for older deletions, formatted drives, or FAT/exFAT media."
            ),
            justify="left",
            wraplength=460,
            text_color="gray75",
        ...
        loading_card = self.make_card(wrapper, "Task Status", "Long recovery tasks show activity here")
    ...
    def _refresh_drives(self) -> None:
        ...
        if self.source_var.get() not in drives:
            self.source_var.set(drives[0])
        if self.destination_var.get() not in drives or self.destination_var.get() == self.source_var.get():
            self.destination_var.set(drives[1] if len(drives) > 1 else drives[0])
    ...
    def _preview_command(self) -> None:
        try:
            ...
            self.logger.write(f"Preview error: {exc}")
    ...
    def _start_recovery(self) -> None:
        if self.loading_indicator.is_running:
            self.logger.write("A recovery task is already running.")
            return

        if not self.recovery_service.is_winfr_available():
            self.logger.write("Windows File Recovery (winfr) was not found. Install it first from the Microsoft Store.")
            return
        ...
        try:
            ...
            self.logger.write(f"Recovery request error: {exc}")
            return
        ...
        TaskRunner.run(
            ...
            on_success=self._on_recovery_success,
            on_error=self._on_recovery_error,
            ui_after=self.after,
        )

    def _on_recovery_success(self, exit_code: int) -> None:
